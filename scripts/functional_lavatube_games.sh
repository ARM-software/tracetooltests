...
echo "<body><h1>Comparison for vulkan games with lavatube</h1><table><tr><th>Name</th><th>Original</th><th>Replay original swapchain</th></tr>" >> $REPORT
...
function replay
{
	...
	echo
	# Replay
	VK_INSTANCE_LAYERS=VK_LAYER_LUNARG_screenshot VK_SCREENSHOT_FRAMES=$3 $LAVATUBE_REPLAYER traces/$1.api
...
function vkquake1
{
	...
	mv vkquake1*.api $TRACEDIR/vkquake1.api
...
function vkquake2
{
	...
	echo "** trace **"
	echo
	LAVATUBE_DEBUG_FILE=$REPORTDIR/vkquake2.txt LAVATUBE_DESTINATION=vkquake2 VK_LAYER_PATH=$LAVATUBE_PATH/implicit_layer.d LD_LIBRARY_PATH=$LAVATUBE_PATH/implicit_layer.d VK_INSTANCE_LAYERS=VK_LAYER_ARM_lavatube ./quake2 $QUAKE2_OPTS
	mv vkquake2*.api $TRACEDIR/vkquake2.api
