...
OPTIMIZER=${REPLAYER:-"$(which gfxrecon-optimize)"}
...
echo "<body><h1>Comparison for vulkan-samples with gfxreconstruct</h1><table><tr><th>Name</th><th>Original</th><th>Replay -m rebind</th><th>Replay trim</th></tr>" >> $REPORT
...
function run
{
	...
	$OPTIMIZER external/vulkan-samples/sample_$1*.gfxr $TRACEDIR/sample_$1.gfxr
	rm -f external/vulkan-samples/*.gfxr
