...
// Reuse the raytracing_1 shaders, but give the embedded arrays unique symbols in
// this translation unit so both tests can link at the same time.
#define vulkan_raytracing_1_raygen_spv vulkan_raytracing_4_raygen_spv
#define vulkan_raytracing_1_raygen_spv_len vulkan_raytracing_4_raygen_spv_len
#include "vulkan_raytracing_1_raygen.rgen.inc"
#undef vulkan_raytracing_1_raygen_spv
#undef vulkan_raytracing_1_raygen_spv_len

#define vulkan_raytracing_1_miss_spv vulkan_raytracing_4_miss_spv
#define vulkan_raytracing_1_miss_spv_len vulkan_raytracing_4_miss_spv_len
#include "vulkan_raytracing_1_miss.rmiss.inc"
#undef vulkan_raytracing_1_miss_spv
#undef vulkan_raytracing_1_miss_spv_len

#define vulkan_raytracing_1_closesthit_spv vulkan_raytracing_4_closesthit_spv
#define vulkan_raytracing_1_closesthit_spv_len vulkan_raytracing_4_closesthit_spv_len
#include "vulkan_raytracing_1_closesthit.rchit.inc"
#undef vulkan_raytracing_1_closesthit_spv
#undef vulkan_raytracing_1_closesthit_spv_len
...
{
	printf("Minimal ray tracing indirect pipeline test using vkCmdSetRayTracingPipelineStackSizeKHR and vkCmdTraceRaysIndirectKHR\n");
}
...
static void create_storage_image(const vulkan_setup_t& vulkan, Resources& resources, uint32_t width, uint32_t height)
{
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_IMAGE, (uint64_t)resources.storage_image, "raytracing_4_storage_image");
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_IMAGE_VIEW, (uint64_t)resources.storage_view, "raytracing_4_storage_view");
...
static void create_ubo(const vulkan_setup_t& vulkan, Resources& resources)
{
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_BUFFER, (uint64_t)resources.ubo.handle, "raytracing_4_camera_buffer");
}
...
static void create_descriptor_set(const vulkan_setup_t& vulkan, Resources& resources)
{
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, (uint64_t)resources.descriptor_set_layout, "raytracing_4_descriptor_layout");
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_PIPELINE_LAYOUT, (uint64_t)resources.pipeline_layout, "raytracing_4_pipeline_layout");
...
static void create_pipeline(const vulkan_setup_t& vulkan, Resources& resources)
{
	resources.shader_stages[0] = acceleration_structures::prepare_shader_stage_create_info(
		vulkan, vulkan_raytracing_4_raygen_spv, vulkan_raytracing_4_raygen_spv_len, VK_SHADER_STAGE_RAYGEN_BIT_KHR);
	resources.shader_stages[1] = acceleration_structures::prepare_shader_stage_create_info(
		vulkan, vulkan_raytracing_4_miss_spv, vulkan_raytracing_4_miss_spv_len, VK_SHADER_STAGE_MISS_BIT_KHR);
	resources.shader_stages[2] = acceleration_structures::prepare_shader_stage_create_info(
		vulkan, vulkan_raytracing_4_closesthit_spv, vulkan_raytracing_4_closesthit_spv_len, VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR);
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_PIPELINE, (uint64_t)resources.pipeline, "raytracing_4_pipeline");
}
...
static void create_sbt(const vulkan_setup_t& vulkan, Resources& resources)
{
	...
	const uint32_t sbt_size = hit_offset + entry_size;

	std::vector<uint8_t> handle_storage(3 * handle_size);
	check(resources.context.functions.vkGetRayTracingShaderGroupHandlesKHR(
		vulkan.device, resources.pipeline, 0, 3, handle_storage.size(), handle_storage.data()));
	...
	resources.sbt_buffer = acceleration_structures::prepare_buffer(
		...
		VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
	test_set_name(vulkan, VK_OBJECT_TYPE_BUFFER, (uint64_t)resources.sbt_buffer.handle, "raytracing_4_sbt");
	...
	testFlushMemoryShaderGroupHandles(
		...
		0,
		sbt_size,
		{raygen_offset, miss_offset, hit_offset},
		{
	...
	resources.callable_region.deviceAddress = 0;
	resources.callable_region.stride = 0;
	resources.callable_region.size = 0;
}

static void create_indirect_buffer(const vulkan_setup_t& vulkan, Resources& resources)
{
	VkTraceRaysIndirectCommandKHR indirect_command{};
	...
	assert((resources.indirect_address & 0x3u) == 0 && "TraceRaysIndirect address must be 4-byte aligned");
	test_set_name(vulkan, VK_OBJECT_TYPE_BUFFER, (uint64_t)resources.indirect_buffer.handle, "raytracing_4_indirect");
}

static uint32_t calculate_pipeline_stack_size(const vulkan_setup_t& vulkan, Resources& resources)
{
	const VkDeviceSize raygen_stack = resources.context.functions.vkGetRayTracingShaderGroupStackSizeKHR(
		vulkan.device, resources.pipeline, 0, VK_SHADER_GROUP_SHADER_GENERAL_KHR);
	const VkDeviceSize miss_stack = resources.context.functions.vkGetRayTracingShaderGroupStackSizeKHR(
...
static void trace(const vulkan_setup_t& vulkan, Resources& resources)
{
	...
	resources.context.functions.vkCmdSetRayTracingPipelineStackSizeKHR(resources.context.command_buffer, resources.stack_size);
	test_marker_mention(vulkan, "Tracing rays indirectly", VK_OBJECT_TYPE_BUFFER, (uint64_t)resources.indirect_buffer.handle);

	resources.context.functions.vkCmdTraceRaysIndirectKHR(
...
int main(int argc, char** argv)
{
	...
	ray_tracing_features.rayTracingPipelineTraceRaysIndirect = VK_TRUE;
	...
	vulkan_setup_t vulkan = test_init(argc, argv, "vulkan_raytracing_4", reqs);
	...
	assert(rt_supported.rayTracingPipelineTraceRaysIndirect && "TraceRaysIndirect feature required");
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_COMMAND_POOL, (uint64_t)resources.context.command_pool, "raytracing_4_command_pool");
	test_set_name(vulkan, VK_OBJECT_TYPE_COMMAND_BUFFER, (uint64_t)resources.context.command_buffer, "raytracing_4_command_buffer");
	...
	resources.stack_size = calculate_pipeline_stack_size(vulkan, resources);
