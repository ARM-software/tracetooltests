...
static GLuint vpos_obj, vcol_obj, vao, fb, tex, draw_program, vs, fs, rb;
...
static int setupGraphics(TOOLSTEST *handle)
{
	...
	glTexStorage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 4, internalformat, handle->width, handle->height, GL_TRUE);
	...
	glRenderbufferStorageMultisample(GL_RENDERBUFFER, max_samples, internalformat, handle->width, handle->height);
	GLint numSamples = 0;
	...
	{
		ELOG("GL_FRAMEBUFFER_COMPLETE failed");
	}
...
static void callback_draw(TOOLSTEST *handle)
{
	...
	glBlitFramebuffer(0, 0, handle->width, handle->height, 0, 0, handle->width, handle->height, GL_COLOR_BUFFER_BIT, GL_NEAREST);
...
{
	return init(argc, argv, "multisample_1", callback_draw, setupGraphics, test_cleanup);
}
