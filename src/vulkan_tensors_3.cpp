// Minimal data graph test based on Vulkan-Samples simple_tensor_and_data_graph.
// Shader generated with:
//   xxd -i -n vulkan_tensors_3_pooling_spv content/vulkan-samples/shaders/tensor_and_data_graph/simple_tensor_and_data_graph/spirv/pooling.spvasm.spv > src/vulkan_tensors_3_pooling.inc
...
#include "vulkan_tensors_3_pooling.inc"
...
static data_graph_session create_data_graph_session(const vulkan_setup_t& vulkan, const data_graph_functions& f, VkPipeline pipeline)
{
	...
	for (const auto& requirement : requirements)
	{
		...
		for (uint32_t object_index = 0; object_index < requirement.numObjects; ++object_index)
		{
			...
			session.sizes.push_back(memreq.memoryRequirements.size);
		}
	}
	return session;
}
...
int main(int argc, char** argv)
{
	...
	vulkan_setup_t vulkan = test_init(argc, argv, "vulkan_tensors_3", reqs);
	...
	const std::array<int64_t, 4> input_dims = { 1, 10, 10, 3 };
	const std::array<int64_t, 4> output_dims = { 1, 5, 5, 3 };

	tensor_resource input_tensor = create_tensor(
	    vulkan, tensor_funcs, input_dims, VK_TENSOR_USAGE_DATA_GRAPH_BIT_ARM,
	    VK_TENSOR_TILING_OPTIMAL_ARM, VK_FORMAT_R32_SFLOAT, 0);
	tensor_resource output_tensor = create_tensor(
	    vulkan, tensor_funcs, output_dims, VK_TENSOR_USAGE_DATA_GRAPH_BIT_ARM,
	    VK_TENSOR_TILING_OPTIMAL_ARM, VK_FORMAT_R32_SFLOAT, 0);
	...
	VkShaderModule graph_shader = create_shader_module(vulkan, vulkan_tensors_3_pooling_spv, vulkan_tensors_3_pooling_spv_len);
