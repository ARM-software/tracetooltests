...
int main(int argc, char** argv)
{
	...
	if (vulkan.vkAssertBuffer)
	{
		...
		(void)child_crc;
		(void)result;
		const VkUpdateBufferInfoARM alien_info{VK_STRUCTURE_TYPE_UPDATE_BUFFER_INFO_ARM, nullptr, alien, 0, VK_WHOLE_SIZE, nullptr};
		result = vulkan.vkAssertBuffer(vulkan.device, &alien_info, &alien_crc, "aliased buffer");
		assert(result == VK_SUCCESS);
		assert(child_crc == orig_crc_alien);
		(void)alien_crc;
		(void)result;
