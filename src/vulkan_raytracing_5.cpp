...
// Procedural hit-group shaders for opcode coverage.
// glslangValidator -V raygen.rgen -o raygen.rgen.spv
// xxd -i -n vulkan_raytracing_5_raygen_spv raygen.rgen.spv > vulkan_raytracing_5_raygen.rgen.inc
#include "vulkan_raytracing_5_raygen.rgen.inc"
// glslangValidator -V miss.rmiss -o miss.rmiss.spv
// xxd -i -n vulkan_raytracing_5_miss_spv miss.rmiss.spv > vulkan_raytracing_5_miss.rmiss.inc
#include "vulkan_raytracing_5_miss.rmiss.inc"
// glslangValidator -V intersection.rint -o intersection.rint.spv
// xxd -i -n vulkan_raytracing_5_intersection_spv intersection.rint.spv > vulkan_raytracing_5_intersection.rint.inc
#include "vulkan_raytracing_5_intersection.rint.inc"
// glslangValidator -V anyhit.rahit -o anyhit.rahit.spv
// xxd -i -n vulkan_raytracing_5_anyhit_spv anyhit.rahit.spv > vulkan_raytracing_5_anyhit.rahit.inc
#include "vulkan_raytracing_5_anyhit.rahit.inc"
// glslangValidator -V closesthit.rchit -o closesthit.rchit.spv
// xxd -i -n vulkan_raytracing_5_closesthit_spv closesthit.rchit.spv > vulkan_raytracing_5_closesthit.rchit.inc
#include "vulkan_raytracing_5_closesthit.rchit.inc"

namespace
{
constexpr uint32_t kWidth = 64;
constexpr uint32_t kHeight = 32;
constexpr const char* kImageFilename = "raytracing_5.png";
...
struct Resources
{
	...
	VkRayTracingShaderGroupCreateInfoKHR shader_groups[kShaderGroupCount]{};
...
{
	printf("Procedural ray tracing test covering OpReportIntersectionKHR, OpIgnoreIntersectionKHR and OpTerminateRayKHR\n");
...
static void create_storage_image(const vulkan_setup_t& vulkan, Resources& resources)
{
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_IMAGE, (uint64_t)resources.storage_image, "raytracing_5_storage_image");
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_IMAGE_VIEW, (uint64_t)resources.storage_view, "raytracing_5_storage_view");
...
static void create_pipeline(const vulkan_setup_t& vulkan, Resources& resources)
{
	resources.shader_stages[0] = acceleration_structures::prepare_shader_stage_create_info(
		vulkan, vulkan_raytracing_5_raygen_spv, vulkan_raytracing_5_raygen_spv_len, VK_SHADER_STAGE_RAYGEN_BIT_KHR);
	resources.shader_stages[1] = acceleration_structures::prepare_shader_stage_create_info(
		vulkan, vulkan_raytracing_5_miss_spv, vulkan_raytracing_5_miss_spv_len, VK_SHADER_STAGE_MISS_BIT_KHR);
	resources.shader_stages[2] = acceleration_structures::prepare_shader_stage_create_info(
		vulkan, vulkan_raytracing_5_intersection_spv, vulkan_raytracing_5_intersection_spv_len, VK_SHADER_STAGE_INTERSECTION_BIT_KHR);
	resources.shader_stages[3] = acceleration_structures::prepare_shader_stage_create_info(
		vulkan, vulkan_raytracing_5_anyhit_spv, vulkan_raytracing_5_anyhit_spv_len, VK_SHADER_STAGE_ANY_HIT_BIT_KHR);
	resources.shader_stages[4] = acceleration_structures::prepare_shader_stage_create_info(
		vulkan, vulkan_raytracing_5_closesthit_spv, vulkan_raytracing_5_closesthit_spv_len, VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR);
	...
	resources.shader_groups[2].type = VK_RAY_TRACING_SHADER_GROUP_TYPE_PROCEDURAL_HIT_GROUP_KHR;
	resources.shader_groups[2].generalShader = VK_SHADER_UNUSED_KHR;
	resources.shader_groups[2].closestHitShader = 4;
	resources.shader_groups[2].anyHitShader = 3;
	resources.shader_groups[2].intersectionShader = 2;
	...
	pipeline_info.groupCount = kShaderGroupCount;
...
static void create_sbt(const vulkan_setup_t& vulkan, Resources& resources)
{
	...
	std::vector<uint8_t> handle_storage(kShaderGroupCount * handle_size);
	check(resources.context.functions.vkGetRayTracingShaderGroupHandlesKHR(
		vulkan.device, resources.pipeline, 0, kShaderGroupCount, handle_storage.size(), handle_storage.data()));
...
static void verify_output(const vulkan_setup_t& vulkan, const vulkan_req_t& reqs, Resources& resources)
{
	...
	{
		filename = kImageFilename;
	}

	const std::vector<uint8_t> pixels = ray_tracing_common::readback_storage_image(
		vulkan, resources.context, resources.storage_image, VK_IMAGE_LAYOUT_GENERAL,
		resources.storage_format, kWidth, kHeight, filename);

	assert(pixels.size() == static_cast<size_t>(kWidth) * kHeight * 4);
	for (uint32_t y = 0; y < kHeight; ++y)
	{
		for (uint32_t x = 0; x < kWidth; ++x)
		{
			const size_t base = (static_cast<size_t>(y) * kWidth + x) * 4;
			const std::array<uint8_t, 4> expected = (x < (kWidth / 2))
				? std::array<uint8_t, 4>{0, 0, 255, 255}
				: std::array<uint8_t, 4>{255, 255, 0, 255};
			for (size_t c = 0; c < expected.size(); ++c)
			{
				if (pixels[base + c] != expected[c])
				{
					fprintf(stderr, "Unexpected output byte (%u,%u,%zu): got %u expected %u\n",
						x, y, c, pixels[base + c], expected[c]);
					assert(false);
				}
			}
		}
...
int main(int argc, char** argv)
{
	...
	vulkan_setup_t vulkan = test_init(argc, argv, "vulkan_raytracing_5", reqs);
	...
	ray_tracing_common::build_simple_aabb_as(vulkan, resources.context, resources.accel);
	create_storage_image(vulkan, resources);
	create_descriptor_set(vulkan, resources);
	create_pipeline(vulkan, resources);
	create_sbt(vulkan, resources);
	trace(vulkan, resources);
	verify_output(vulkan, reqs, resources);
