...
#define vulkan_raytracing_1_raygen_spv vulkan_raytracing_3_raygen_spv
#define vulkan_raytracing_1_raygen_spv_len vulkan_raytracing_3_raygen_spv_len
#include "vulkan_raytracing_1_raygen.rgen.inc"
#undef vulkan_raytracing_1_raygen_spv
#undef vulkan_raytracing_1_raygen_spv_len

#define vulkan_raytracing_1_miss_spv vulkan_raytracing_3_miss_spv
#define vulkan_raytracing_1_miss_spv_len vulkan_raytracing_3_miss_spv_len
#include "vulkan_raytracing_1_miss.rmiss.inc"
#undef vulkan_raytracing_1_miss_spv
#undef vulkan_raytracing_1_miss_spv_len

#define vulkan_raytracing_1_closesthit_spv vulkan_raytracing_3_closesthit_spv
#define vulkan_raytracing_1_closesthit_spv_len vulkan_raytracing_3_closesthit_spv_len
...
{
	printf("Minimal ray tracing indirect pipeline test using vkCmdTraceRaysIndirect2KHR\n");
}
...
static void create_storage_image(const vulkan_setup_t& vulkan, Resources& resources, uint32_t width, uint32_t height)
{
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_IMAGE, (uint64_t)resources.storage_image, "raytracing_3_storage_image");
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_IMAGE_VIEW, (uint64_t)resources.storage_view, "raytracing_3_storage_view");
...
static void create_ubo(const vulkan_setup_t& vulkan, Resources& resources)
{
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_BUFFER, (uint64_t)resources.ubo.handle, "raytracing_3_camera_buffer");
}
...
static void create_descriptor_set(const vulkan_setup_t& vulkan, Resources& resources)
{
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, (uint64_t)resources.descriptor_set_layout, "raytracing_3_descriptor_layout");
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_PIPELINE_LAYOUT, (uint64_t)resources.pipeline_layout, "raytracing_3_pipeline_layout");
...
static void create_pipeline(const vulkan_setup_t& vulkan, Resources& resources)
{
	resources.shader_stages[0] = acceleration_structures::prepare_shader_stage_create_info(
		vulkan, vulkan_raytracing_3_raygen_spv, vulkan_raytracing_3_raygen_spv_len, VK_SHADER_STAGE_RAYGEN_BIT_KHR);
	resources.shader_stages[1] = acceleration_structures::prepare_shader_stage_create_info(
		vulkan, vulkan_raytracing_3_miss_spv, vulkan_raytracing_3_miss_spv_len, VK_SHADER_STAGE_MISS_BIT_KHR);
	resources.shader_stages[2] = acceleration_structures::prepare_shader_stage_create_info(
		vulkan, vulkan_raytracing_3_closesthit_spv, vulkan_raytracing_3_closesthit_spv_len, VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR);
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_PIPELINE, (uint64_t)resources.pipeline, "raytracing_3_pipeline");
}
...
static void create_sbt(const vulkan_setup_t& vulkan, Resources& resources)
{
	...
	resources.sbt_buffer = acceleration_structures::prepare_buffer(
		...
		VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
	test_set_name(vulkan, VK_OBJECT_TYPE_BUFFER, (uint64_t)resources.sbt_buffer.handle, "raytracing_3_sbt");
...
static void create_indirect_buffer(const vulkan_setup_t& vulkan, Resources& resources)
{
	...
	indirect_command.raygenShaderRecordAddress = resources.raygen_region.deviceAddress;
	indirect_command.raygenShaderRecordSize = resources.raygen_region.size;
	indirect_command.missShaderBindingTableAddress = resources.miss_region.deviceAddress;
	indirect_command.missShaderBindingTableSize = resources.miss_region.size;
	indirect_command.missShaderBindingTableStride = resources.miss_region.stride;
	indirect_command.hitShaderBindingTableAddress = resources.hit_region.deviceAddress;
	indirect_command.hitShaderBindingTableSize = resources.hit_region.size;
	indirect_command.hitShaderBindingTableStride = resources.hit_region.stride;
	indirect_command.callableShaderBindingTableAddress = resources.callable_region.deviceAddress;
	indirect_command.callableShaderBindingTableSize = resources.callable_region.size;
	indirect_command.callableShaderBindingTableStride = resources.callable_region.stride;
...
int main(int argc, char** argv)
{
	...
	vulkan_setup_t vulkan = test_init(argc, argv, "vulkan_raytracing_3", reqs);
	...
	test_set_name(vulkan, VK_OBJECT_TYPE_COMMAND_POOL, (uint64_t)resources.context.command_pool, "raytracing_3_command_pool");
	test_set_name(vulkan, VK_OBJECT_TYPE_COMMAND_BUFFER, (uint64_t)resources.context.command_buffer, "raytracing_3_command_buffer");
